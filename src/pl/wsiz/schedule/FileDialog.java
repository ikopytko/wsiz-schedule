...
public class FileDialog extends ListActivity {
	...
	public int MODE_CREATE = 0;

	public int MODE_OPEN = 1;
	...
	private void getDirImpl(final String dirPath) {
		...
		for (File file : files) {
			if (file.isDirectory()) {
				...
				final String fileNameLwr = fileName.toLowerCase(getResources().getConfiguration().locale);
				// se ha um filtro de formatos, utiliza-o
				if (formatFilter != null) {
					boolean contains = false;
					for (int i = 0; i < formatFilter.length; i++) {
						final String formatLwr = formatFilter[i].toLowerCase(getResources().getConfiguration().locale);
